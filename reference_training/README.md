...
For an executable reproduction of the archived numerical results, use `scripts/reproduce_results.py`. For the configuration and exact source hashes, see `protocols/training_config.json`. Complete model retraining remains outside this release. A portable frozen SideView inference adapter is now included in `scripts/score_sideview.py`; it requires all 45 original checkpoints, which are not bundled.
